...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--conv_dim', type=int, default=64, help='number of conv filters in the first layer of the UNet'
    )
    ...
    parser.add_argument(
        '--selected_attrs', '--list', nargs='+', help='selected attributes/domains', default=['rain', 'fog', 'night']
    )  # rain, fog, night
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.008,
        help='Sharpness parameter, controls how "sudden" the change is. A lower value makes the cosine curve smoother.'
    )
    parser.add_argument(
        '--cos_scheduler',
        type=int,
        default=0,
        choices=[0, 1],
        help='enable cosine scheduler, default is linear scheduler (0 for False, 1 for True)'
    )
    ...
    parser.add_argument(
        '--wandb', type=int, default=0, choices=[0, 1], help='enable wandb logging (0 for False, 1 for True)'
    )

    # Directories.
    parser.add_argument(
        '--image_dir', type=str, default='/media/talmacsi/48a93eb4-f27d-48ec-9f74-64e475c3b6ff/Downloads/data/rgb_anon'
    )
    parser.add_argument('--log_dir', type=str, default='diffusion_model/outputs/logs')
    parser.add_argument('--model_save_dir', type=str, default='diffusion_model/outputs/checkpoints')
    parser.add_argument('--sample_dir', type=str, default='diffusion_model/outputs/samples')
    parser.add_argument('--result_dir', type=str, default='diffusion_model/outputs/results')
