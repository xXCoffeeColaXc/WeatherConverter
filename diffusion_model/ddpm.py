...
class Diffusion:
    ...
    def ddpm_sample(self, n):
        ...
        with torch.no_grad():
            ...
            for i in tqdm(reversed(range(1, self.config.noise_steps)), position=0):
                t = (torch.ones(n) *
                     i).long().to(self.config.device)  # create a tensor of lenght n with the current timestep
    ...
    def test(self):
        ...
        if self.config.wandb:
            print("wandb log")
            wandb.log({"kid_score": kid_score, "fid_score": fid_score})
    ...
    def save_checkpoint(self, epoch):
        ...
        """
        save_dict = {
            'epoch': epoch, 'model_state_dict': self.unet.state_dict(), 'optimizer_state_dict': self.opt.state_dict()
        }
    ...
    def setup_logger(self):
        ...
        # Initialize WandB
        wandb.init(
            project='bird-diffusion-project',
            config={
                "image_size": self.config.image_size,
                "block_depth": self.config.block_depth,
                "time_emb_dim": self.config.time_emb_dim,
                ...
                "lr": self.config.lr,
                "noise_steps": self.config.noise_steps,  # ... Add other hyperparameters here
            }
        )
