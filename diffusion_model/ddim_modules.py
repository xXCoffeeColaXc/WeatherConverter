...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        K = self.keys(x).view(batch_size, seq_length, self.num_heads,
                              self.head_dim)
        Q = self.queries(x).view(batch_size, seq_length, self.num_heads,
                                 self.head_dim)
        V = self.values(x).view(batch_size, seq_length, self.num_heads,
                                self.head_dim)

        # Compute attention scores
        energy = torch.einsum("nqhd,nkhd->nhqk", [Q, K]) / (self.embed_size
                                                            **(1 / 4))
        attention = torch.softmax(energy, dim=-1)

        # Apply attention to values and concatenate heads
        out = torch.einsum("nhql,nlhd->nqhd",
                           [attention, V]).reshape(batch_size, seq_length,
                                                   embed_size)
        out = self.fc_out(out)
...
class ResidualBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.double_conv = nn.Sequential(
            nn.BatchNorm2d(
                in_channels
            ),  #TODO Try other normalization (group), BATCHNORM is the worst
...
class DownBlock(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(block_depth):
            if i == 0:
                layers.append(
                    ResidualBlock(in_channels, out_channels, residual=True))
            else:
                layers.append(
                    ResidualBlock(out_channels, out_channels, residual=False))
...
class UpBlock(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(block_depth):
            if i == 0:
                layers.append(
                    ResidualBlock(in_channels + skip_channels,
                                  out_channels,
                                  residual=True))
            else:
                layers.append(
                    ResidualBlock(out_channels + skip_channels,
                                  out_channels,
                                  residual=True))
...
class UNet(DiffusionUNet):
    ...
    def __init__(self,
        ...
        self.pre_conv = nn.Conv2d(c_in,
                                  ...
                                  padding=1,
                                  bias=False)
        self.embedding_upsample = nn.Upsample(size=(image_size, image_size),
                                              mode='nearest')
        ...
        self.bottleneck2 = ResidualBlock(
            256, 256, residual=True)  # TODO: Deeper, wider net, for detail
